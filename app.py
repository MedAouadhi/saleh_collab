    ...
    with app.app_context():
        ...
        if not User.query.first():
            ...
            for username in new_users:
                ...
                new_user = User(username=username, password=hashed_password, is_admin=False); database.session.add(new_user)
                print(f"  Added user: {username} (password: {password})")
            # No default episodes seeded anymore
            database.session.commit()
            print("Initial users seeded. No default episodes added.")
        else: print("Database already contains data.")

# --- Custom Admin Forms & Setup ---
...
class EpisodeAdminView(SecureModelView):
    column_list = ('id', 'title', 'display_order', 'last_updated', 'assignees') # Added display_order
    column_searchable_list = ('title', 'plan', 'scenario')
    form_excluded_columns = ('comments', 'assignees')
    column_display_pk = True
    column_sortable_list = ('id', 'title', 'last_updated', 'display_order') # Make order sortable
...
@login_required
def dashboard():
    """Displays the dashboard with all episodes ordered by display_order."""
    app.logger.info(f"Accessing dashboard route for user: {current_user.username}")
    try:
        # --- UPDATED QUERY: Order by display_order ---
        all_episodes = Episode.query.options(
            joinedload(Episode.assignees)
        ).order_by(Episode.display_order, Episode.id).all() # Added id as secondary sort
...
@login_required
def create_episode():
    """Creates a new episode and assigns the creator. Sets initial display order."""
    ...
    try:
        # --- Set initial display order (e.g., based on current max + 1 or timestamp) ---
        # Simple approach: use current max ID + 1 (assuming IDs are sequential)
        # A more robust approach might use timestamps or a dedicated sequence
        last_episode = Episode.query.order_by(Episode.id.desc()).first()
        initial_order = (last_episode.id + 1) * 10 if last_episode else 0 # Multiply to leave space
        # Or simply use 0 and let drag-and-drop sort it out initially

        new_episode = Episode(title=title, display_order=initial_order)
...
# --- NEW ROUTE: Update Episode Order ---
@app.route('/api/update_episode_order', methods=['POST'])
@login_required
def update_episode_order():
    """Receives the new order of episode IDs and updates the database."""
    ...
    try:
        # Update display_order based on the received list index
        for index, episode_id_str in enumerate(ordered_ids):
            try:
                episode_id = int(episode_id_str)
                episode = Episode.query.get(episode_id)
                if episode:
                    episode.display_order = index # Set order based on position in list
                    db.session.add(episode) # Add to session to mark as dirty
                else:
                    app.logger.warning(f"Episode ID {episode_id} not found during reorder.")
            except ValueError:
        ...
        return jsonify({'success': False, 'message': 'حدث خطأ أثناء تحديث الترتيب.'}), 500
